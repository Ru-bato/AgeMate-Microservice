<template>
  <div class="home-page">
    <div class="welcome-message">
      <!-- 循环展示所有日志 -->
      <el-card class="card" v-for="(log, index) in logs" :key="index" style="width: 1200px" shadow="hover">
        ...
        <div class="card-footer">
          ...
          <el-button type="danger" @click="deleteLog(log.log_id)">删除</el-button> <!-- 添加删除按钮 -->
        </div>
      </el-card>
    </div>
...
// 存储获取的日志数据
const logs = ref<Array<{ log_id: string, title: string, content: string }>>([]); // 使用ref来存储响应式数据
...
const openMarkdownDialog = (log: { log_id: string, content: string }) => {
  markdownContent.value = marked(log.content); // 转换Markdown为HTML
...
const addFavorite = async (log: { log_id: string }) => {
  try {
    const userId: number = 1; // 显式声明为 number 类型
    const response = await axios.post("http://127.0.0.1:8000/logs/favorites/", null, {
      params: {
        user_id: userId,      // 作为查询参数传递
        log_id: log.log_id    // 作为查询参数传递
      }
    });
