...
const openMarkdownDialog = (log: { log_id: string, title: string, content: string }) => {
  markdownContent.value = marked(log.content).toString(); // 将Markdown内容转换为HTML
  currentLog.value = log; // 保存当前日志
  dialogVisible.value = true; // 打开弹窗
};

// 取消收藏操作
const removeFavorite = async (log: { log_id: string; title: string; content: string; } | null) => {
  if (!log) return; // 添加空值检查
