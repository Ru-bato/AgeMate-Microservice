...
async def take_screenshot(path: str, clip=None):
    ...
    await websocket_manager.broadcast_action(screenshot_data)
    # TODO: 等待扩展返回截图
    await asyncio.sleep(1)  # 临时方案，后续改为等待确认
...
async def main(config, base_dir, query_: str, url_: str, clientIP: str) -> None:
    # 连接到WebSocket服务器
    # logger_.info("已连接到WebSocket服务器")
    ...
    for single_query_task in query_tasks:
        ...
        try:
            async with async_playwright() as playwright:
                ...
                try:
                    # 连接到用户浏览器的调试端口
                    browser = await playwright.chromium.connect_over_cdp(f"ws://{clientIP}:9222")
                    # 获取所有页面
