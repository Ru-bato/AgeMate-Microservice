...
The script is designed to automate complex web qinginteractions, enhancing accessibility and efficiency in web navigation tasks.
"""
...
async def main(config, base_dir, query_: str, url_: str) -> None:
    logger_.info(config)
    # basic settings
        ...
        try:
            async with async_playwright() as playwright:
                ...
                try:
                    ...
                    logger.info(f"成功连接到浏览器: {browser}")

                    # 获取所有上下文
                    contexts = browser.contexts
                    logger.info(f"浏览器上下文数量: {len(contexts)}")

                    if contexts:
                        # 使用第一个上下文
                        session_control.context = contexts[0]
                        logger.info(f"使用第一个上下文: {session_control.context}")

                        # 获取上下文中的所有页面，并过滤掉扩展程序页面
                        pages = [page for page in session_control.context.pages if not page.url.startswith("chrome-extension://")]
                        logger.info(f"上下文中的页面数量 (过滤后): {len(pages)}")

                        # 检查用户是否已经打开了目标 URL 的页面
                        target_page = None
                        for page in pages:
                            if page.url == confirmed_website_url:
                                target_page = page
                                break

                        if target_page:
                            # 如果用户已经打开了目标 URL 的页面，使用该页面
                            session_control.active_page = target_page
                            logger.info(f"使用已存在的页面: {session_control.active_page.url}")
                        else:
                            # 如果用户没有打开目标 URL 的页面，创建一个新页面并导航到目标 URL
                            session_control.active_page = await session_control.context.new_page()
                            logger.info("创建了一个新页面")
                            await session_control.active_page.goto(confirmed_website_url, wait_until="load")
                            logger.info(f"导航到: {confirmed_website_url}")
                    else:
                        # 如果没有上下文，创建一个新上下文
                        session_control.context = await browser.new_context()
                        logger.info("创建了一个新的浏览器上下文")
                        session_control.active_page = await session_control.context.new_page()
                        logger.info("在上下文中创建了一个新页面")
                        await session_control.active_page.goto(confirmed_website_url, wait_until="load")
                        logger.info(f"导航到: {confirmed_website_url}")
                ...
                if session_control.active_page and session_control.active_page.viewport_size:
                    total_width = session_control.active_page.viewport_size["width"]
                else:
                    # 如果 viewport_size 为 None，设置一个默认值或者根据你的需求进行处理
                    total_width = 1280  # 设置默认宽度为 1280
                    logger.warning("viewport_size 为 None, 使用默认宽度 1280")

                # 移除不必要的代码：await session_control.context.new_page()

                # 你可能不需要再次调用 goto，除非你想确保页面被重新加载
                # try:
                #     await session_control.active_page.goto(confirmed_website_url, wait_until="load")
                # except Exception as e:
                ...
                # TODO: Why sleep???
                await asyncio.sleep(3) # 这一行需要根据你的实际需求来决定是否保留
                ...
                while not complete_flag:
                    ...
                    if ranker_path and len(elements) > top_k:
                        ...
                        pred_scores = ranking_model.predict(ranking_input, convert_to_numpy=True,
                    ...
                    for multichoice_i in range(0, num_choices, step_length):
                        ...
                        try:
                            await session_control.active_page.screenshot(path=input_image_path, clip=clip,
                                                                         full_page=True,
                    ...
                    try:
                        ...
                        try:
                            ...
                            elif monitor_signal == "pause":  # What's this???
                                logger.info(
                                    "Pause for human intervention. Press Enter to continue. You can also enter your message here, which will be included in the action history as a human message.")
                                # TODO: 这里需要修改为从websocket中获取指令
                                #  human_intervention = await ainput()
                                human_intervention = "Please continue"
                                if human_intervention:
            ...
            # TODO: 断开与WebSocket服务器的连接

            logger_.info("已断开与WebSocket服务器的连接")
